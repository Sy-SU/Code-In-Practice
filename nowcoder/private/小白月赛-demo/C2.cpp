...
void solve() {
	...
	int blklen = sqrt(500 * n), blknum = (n + blklen - 1) / blklen;
	...
	std::vector<std::vector<i64>> block(blknum + 1, std::vector<i64>(1001));
	...
	while (q--) {
		...
		if (op == 1) {
			block[blkInd(x)][f[x]]--, block[blkInd(x)][fr(y)]++;
			a[x] = y, f[x] = fr(y);
		} else {
			std::vector<i64> cnt(1001);
