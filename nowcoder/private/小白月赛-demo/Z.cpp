...
void solve() {
	...
	std::vector<int> fa(n + 1), dep(n + 1), son(n + 1), sz(n + 1), top(n + 1), w(n + 1); // w(u) 表示 w(u, p(u))

	auto dfs1 = [&](auto &&self, int u, int f) -> void {
		fa[u] = f, dep[u] = dep[f] + 1, sz[u] = 1;
		for (const auto &[v, wei] : edge[u]) {
			if (v == f) {
				continue;
			}
			self(self, v, u);
	...
	auto dfs3 = [&](auto &&self, int u, int f) -> void {
		...
		for (const auto &[v, wei] : edge[u]) {
			...
			self(self, v, u);
			down[u] += down[v], sumdown[u] += sumdown[v];
		}
	...
	auto dfs4 = [&](auto &&self, int u, int f) -> void {
		for (const auto &[v, wei] : edge[u]) {
			...
			up[v] = a[u] + sumdown[u] + up[u] - down[v] - 2 * wei;
			self(self, v, u);
		}
	};
	...
	auto dfs5 = [&](auto &&self, int u, int f) -> void {
		...
		preedge[u] = preedge[f] + (valedge[u] = down[u] + up[u] + w[u]);
		for (const auto &[v, wei] : edge[u]) {
			if (v == f) {
				continue;
			}
