...
void solve() {
	...
	SegmentTree<Info, Tag> index(dp), value(dp);

	for (int i = 1; i <= n; i++) {
		auto [vl, id] = vec[i];
		dp[i] = std::max({index.query(id, n).max, value.query(1, vl - 2).max, value.query(vl, n).max}) + 1;
		value.modify(vl, vl, dp[i]), index.modify(id, id, dp[i]);
	}
	std::cout << n - *std::max_element(dp.begin() + 1, dp.end()) << '\n';
}	
